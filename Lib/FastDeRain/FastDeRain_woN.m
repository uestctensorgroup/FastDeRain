...
function [B,R,iter] = FastDeRain_woN(O,opts)
%%%--- Preparation ---%%%
...
R = zeros(Size); 
B = O; 
D1 = zeros(Size);      D2 = zeros(Size);
D3 = zeros(Size);      D4 = zeros(Size);

DemonB = mu*ones(Size) + mu*eigsDyTDy + mu*eigsDtTDt + mu*eigsDxTDx;
NormO = norm(O(:),'fro');
%%%--- Main loop---%%%
...
while relcha>tol &&  iter<maxit
    ...
    %%%--- B-subproblem ---%%%
    B = real(ifftn(fftn(mu*DxT(Dx(O)) - mu*DxT(V1+D1) + mu*O -mu*(V2+D2) + mu*DyT(V3+D3) + mu*DtT(V4+D4) )./DemonB));
    B(B<0) = 0;    B(B>O) = O(B>O);
    R_mid = O-B;
    %%%--- R-subproblem ---%%%
    R = R_mid;
